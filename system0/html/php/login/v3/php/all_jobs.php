		...
		<div class="row justify-content-center">
	      ...
	      <h1>All running Jobs</h1>
				<?php
					...
					mysqli_stmt_fetch($stmt);	
					//echo($cnt);
					echo("<div class='container' style='width:95vw'><div class='row'><div class='col'><div class='overflow-auto'><table class='table'><thead><tr><th>Printer</th><th>file</th><th>completion</th><th>free</th><th>cancel print</th><th>detailes</th></tr></thead><tbody>");
					$last_id=0;					
					while($cnt!=0)
					{
						...
						if($progress==100)
							echo("<tr><td>$printer_id</td><td>$file</td><td>$progress%</td><td><form method='POST' action='?free=$printer_id'><button type='submit' value='free' name='free' class='btn btn-dark'>Aufräumen</button> </form></td><td>Job already finished</td><td><form method='POST' action='job_info_all.php'><input type='submit' value='detailes'> </form></td></tr>");
						else if($cancel==1)
							echo("<tr><td>$printer_id</td><td>$file</td><td>cancelled</td><td><form method='POST' action='?free=$printer_id'><button type='submit' value='free' name='free' class='btn btn-dark'>Aufräumen</button> </form></td><td>Job cancelled</td><td><form method='POST' action='job_info_all.php'><input type='submit' value='detailes'> </form></td></tr>");
						else
							echo("<tr><td>$printer_id</td><td>$file</td><td>$progress%</td><td>Job still running</td><td><form method='POST' action='?cancel=$printer_id'><button type='submit' value='cancel' name='cancel' class='btn btn-dark'>Abbrechen</button></form></td><td><form method='POST' action='job_info_all.php'><input type='submit' value='detailes'> </form></td></tr>");
